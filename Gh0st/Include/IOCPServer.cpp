...
unsigned char* MyDecode(unsigned char* data, int len)
{
	for (int i = 0; i < len; i++)
	{
		data[i] ^= 0x119;
...
unsigned char* MyEncode(unsigned char* data, int len)
{
	for (int i = 0; i < len; i++)
	{
		data[i] -= 0x446;
...
*/
	...
	try
	{
		if (nSize > 0)
		{
			...
			{
				MyEncode((PBYTE)pContext->m_WriteBuffer.GetBuffer(),pContext->m_WriteBuffer.GetBufferLen());
				PostSend(pContext);
			}
			else
			{
...
bool CIOCPServer::OnClientReading(ClientContext* pContext, DWORD dwIoSize)
{
	...
	try
	{
		...
		MyDecode((PBYTE)pContext->m_byInBuffer,dwIoSize);
